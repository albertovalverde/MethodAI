...
st.set_page_config(
    page_title="Streamly - An Intelligent MethodAI Assistant",
...
def main():
    ...
    mode = st.sidebar.radio("Select Mode:", options=["Latest Updates", "Chat with MethodAI"], index=1)
    ...
    with st.sidebar:
        st.write("Select an AI Agent:")
        if st.button("AI Agent 1"):
            st.write("Messages from AI Agent 1 here...")
        if st.button("AI Agent 2"):
            st.write("Messages from AI Agent 2 here...")
        if st.button("AI Agent 3"):
            st.write("Messages from AI Agent 3 here...")

    if mode == "Chat with MethodAI":
